...
describe('Station route', () => {
  ...
  test('Should load station page with default station ID 3089', async () => {
  ...
  test('Should accept dataType query parameter', async () => {
    const { result, statusCode } = await server.inject({
      method: 'GET',
      url: '/station?stationId=3089&dataType=forecast',
      headers: {
        cookie: authCookie
      }
    })
  ...
  test('Should accept stationType query parameter', async () => {
    const { result, statusCode } = await server.inject({
      method: 'GET',
      url: '/station?stationId=3089&stationType=river',
      headers: {
        cookie: authCookie
      }
    })
